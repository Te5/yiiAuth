    ...
    public function handle()
    {
        ...
        switch ($this->client->getName()) {
            case 'facebook':
                ...
                break;
            case 'vkontakte':
                $nickname = $attributes['first_name'] . ' ' . $attributes['last_name'];
                break;
            case 'google':
        ...
        if (Yii::$app->user->isGuest) {
            if ($auth) { // login
                /* @var User $user */
                $user = Users::findIdentity($auth->user_id);
/*                print_r($user);
                die();*/
                ...
                if ($email !== null && Users::find()->where(['email' => $email])->exists()) {
                    ...
                    $email? null : $email = 'undefined@email.com';
                    $user = new Users([
                        'login' => $nickname,
                        'username' => $nickname,
                        'email' => $email,
                        'password' => $password,
                        'active' => 1, //для oauth уже активирован
                    ]);
                    $user->scenario = 'oAuthRegister';
                    //$user->generateAuthKey(); не поддерживается
